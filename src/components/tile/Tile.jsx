...
function Tile({ number, moveTile, numberOfTiles }) {
    ...
    return (
        <div
            className={`${styles[`number-${classPrefix}`]} ${number.value === numberOfTiles ? styles.disabled : ''} ${styles[`slot-${classPrefix}-${number.index}`]} ${number.value === number.index + 1 ? styles.correct : ''}`}
